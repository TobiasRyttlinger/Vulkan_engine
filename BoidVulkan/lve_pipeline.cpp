...
namespace lve {
	...
	void LvePipeline::createGraphicsPipeline(const std::string& vertFilePath,const std::string& fragFilePath,const pipelineConfigInfo& configInfo) {
		...
		assert(configInfo.pipelineLayout != VK_NULL_HANDLE && "cannot create graphics pipeline provided in configInfo");
		...
		auto bindDescriptions = LveModel::Vertex::getBindingDescriptions();
		auto attirbuteDescriptions = LveModel::Vertex::getAttributeDescriptions();

		VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
		vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
		vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attirbuteDescriptions.size());
		vertexInputInfo.vertexBindingDescriptionCount = static_cast<uint32_t>(bindDescriptions.size());
		vertexInputInfo.pVertexAttributeDescriptions = attirbuteDescriptions.data();
